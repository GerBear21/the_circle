...
/**
 * Archived documents view — every fully-reviewed (approved) request lands
 * here automatically with its signed PDF. Users can regenerate the PDF on
 * demand and push it into their Microsoft 365 ecosystem (OneDrive + Outlook).
 */
export default function ArchiveView() {
    ...
    useEffect(() => {
        if (status === 'authenticated') {
            // Auto-sync archives on load so newly approved requests appear, then fetch
    ...
    /** Regenerate the signed PDF on demand. */
    const regeneratePdf = async (archive: ArchivedDocument, e: React.MouseEvent) => {
        e.stopPropagation();
        try {
            setGeneratingId(archive.id);
            const resp = await fetch('/api/archives/generate-pdf', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ requestId: archive.request_id, force: true }),
            });
            if (resp.ok) await fetchArchived();
        } catch (err) {
    ...
    /** Push this document into the user's Microsoft 365 (OneDrive + Outlook email). */
    const syncToMicrosoft = async (archive: ArchivedDocument, e: React.MouseEvent) => {
        e.stopPropagation();
        try {
            setSyncingId(archive.id);
            setSyncMessage(null);
            const resp = await fetch('/api/archives/sync-microsoft', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ archiveId: archive.id }),
            });
            const data = await resp.json().catch(() => ({}));
            if (!resp.ok) throw new Error(data.error || 'Microsoft 365 sync failed');
            const sent: string[] = [];
            if (data.onedrive) sent.push('OneDrive');
            if (data.email) sent.push('Outlook (PDF attached)');
            if (data.teams) sent.push('Teams');
            if (data.sharepoint) sent.push('SharePoint');
    ...
    if (loading && !archivedDocuments.length) {
        return (
        ...
        <div className="space-y-6">
            ...
            {syncMessage && (
                <div className="bg-brand-50 border border-brand-200 text-brand-800 rounded-xl px-4 py-3 text-sm flex items-center justify-between">
                    ...
                    <button onClick={() => setSyncMessage(null)} className="text-brand-500 hover:text-brand-700 font-bold ml-4">✕</button>
                </div>
            )}
            ...
            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col lg:flex-row items-center gap-4">
                {/* View Mode Toggle */}
                {!currentFolder && (
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="px-4 py-2.5 bg-brand-50 rounded-xl text-sm font-semibold text-brand-700 whitespace-nowrap">
                        {totalRequests} archived
                    </div>
                </div>
            </div>
            ...
            {viewMode === 'folders' && !currentFolder && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    ...
                    {folders.length === 0 && (
                        <div className="col-span-full flex flex-col items-center justify-center py-16 bg-white rounded-3xl border border-dashed border-gray-300">
                            <h3 className="text-lg font-bold text-gray-900">No archived documents yet</h3>
                            <p className="text-gray-500 mt-2 max-w-sm text-center text-sm">
                                Once a request is fully reviewed and approved, its signed PDF is automatically recorded here.
                            </p>
                        </div>
                    )}
                </div>
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    <button
                                                        onClick={(e) => regeneratePdf(archive, e)}
                                                        disabled={generatingId === archive.id}
                                                    ...
                                                    >
                                                        <RefreshCw className={`w-5 h-5 ${generatingId === archive.id ? 'animate-spin' : ''}`} strokeWidth={1.5} />
                                                    </button>
                                                    <button
                                                        onClick={(e) => syncToMicrosoft(archive, e)}
                                                        disabled={syncingId === archive.id}
                                                        className="p-1.5 rounded-lg text-gray-400 hover:text-brand-600 hover:bg-brand-50 transition-colors disabled:opacity-50"
                                                        title="Send to my Microsoft 365 (OneDrive + Outlook)"
                                                    >
                                                        <CloudUpload className={`w-5 h-5 ${syncingId === archive.id ? 'animate-pulse' : ''}`} strokeWidth={1.5} />
                                                    </button>
                                                    <button
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="border-t border-gray-200 px-6 py-4 bg-gray-50 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={(e) => {
                                        regeneratePdf(selectedArchive, e);
                                    }}
                                    disabled={generatingId === selectedArchive.id}
                                    className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 font-medium text-sm hover:bg-gray-100 transition-colors disabled:opacity-50"
                                >
                                    {generatingId === selectedArchive.id ? 'Generating…' : 'Regenerate PDF'}
                                </button>
                                <button
                                    onClick={(e) => {
