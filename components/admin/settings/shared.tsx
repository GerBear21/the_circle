...
export function CardHeading({ icon, iconBg, iconColor, title }: { icon: React.ReactNode; iconBg?: string; iconColor?: string; title: string }) {
  // Icons are rendered plain and monochrome (no colour chip) to match the
  // side navigation. iconBg/iconColor are accepted for backwards-compat but
  // intentionally ignored.
  return (
    <h3 className="text-lg font-semibold text-text-primary mb-4 flex items-center gap-2.5">
      <span className="text-neutral-700 flex items-center justify-center">{icon}</span>
      {title}
    </h3>
  );
}
...
export function NavButton({ active, label, icon, onClick }: { active: boolean; label: string; icon: any; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all text-sm ${active ? 'bg-primary-50 text-primary-700 font-medium' : 'text-neutral-700 hover:bg-neutral-100 hover:text-neutral-900 font-normal'}`}>
      <span className={active ? 'text-primary-600' : 'text-neutral-700'}>{icon}</span>
      {label}
    </button>
  );
}
...
// Icons
export function ClockIcon() { return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>; }
...
export function FormsIcon() { return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>; }
...
export function DelegationIcon() { return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>; }
