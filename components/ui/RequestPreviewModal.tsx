...
export default function RequestPreviewModal({
    ...
    const handlePrint = () => {
        ...
        if (!area) return;
        // Rewrite the logo src to an absolute URL so it loads in the new window (about:blank origin).
        const clone = area.cloneNode(true) as HTMLDivElement;
        ...
        w.document.write(`<!doctype html><html><head><title>${title}</title>
            ...
            <style>
                ...
                table.doc-grid td, table.doc-grid th { border: 1px solid #333; padding: 6px 8px; vertical-align: top; }
                table.doc-grid th { background: #F3EADC; color: #5E4426; text-align: left; font-weight: 700; }
                .doc-label { font-weight: 700; color: #333; text-transform: uppercase; font-size: 10px; letter-spacing: 0.04em; }
                .doc-value { color: #111; white-space: pre-wrap; font-size: 11px; }
                .subtitle { color: #666; font-size: 11px; margin-bottom: 14px; text-align: center; }
                .conditions { font-size: 10.5px; line-height: 1.5; color: #222; background: #FAF7F0; border: 1px solid #E5DFD2; padding: 10px 12px; }
                .approval-row td { border: 1px solid #333; padding: 8px; vertical-align: top; width: 25%; }
                .sig-line { border-bottom: 1px solid #666; height: 28px; margin-top: 4px; }
                ...
                >
                    ...
                    <div className="max-h-[70vh] overflow-y-auto p-6 bg-gray-50">
                        ...
                        >
                            ...
                            {subtitle && (
                                <p
                                    className="subtitle text-xs text-gray-500 mb-3 text-center"
                                    style={{ textAlign: 'center' }}
                                >
                                    {subtitle}
                                </p>
                                    ...
                                    {section.content ? (
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                                            {section.fields.map((f, j) => (
                                                <div
                                                    key={j}
                                                    className={`field ${f.fullWidth ? 'sm:col-span-2 full' : ''}`}
                                                >
