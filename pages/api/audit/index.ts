...
import { AUDIT_VIEW_PERMISSIONS } from '@/lib/auditAccess';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const { allowed } = await requireAnyPermission(user.id, AUDIT_VIEW_PERMISSIONS);
        if (!allowed) {
            return res.status(403).json({ error: 'Audit access requires the Auditor role or audit permissions' });
        }
